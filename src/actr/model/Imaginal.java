...
/**
 * The imaginal module that holds a temporary problem state for the current task.
 *  
...
public class Imaginal extends Module
{
	...
	void update ()
	{
		...
		if (chunk!=null && chunk.isRequest())
		{
			...
			{ model.outputWarning ("imaginal busy, request ignored"); return; }
			...
			model.addEvent (new Event (model.getTime() + imaginalDelay, "imaginal", "set-buffer ["+chunk.getName()+"]")
			{
				public void action() {
