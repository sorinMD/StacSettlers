...
/**
 * Actions performed on a buffer as specified in the (right-hand) action side of an ACT-R production rule.
 *  
...
class BufferAction
{
	 ...
	 * @return <tt>true</tt> if the two buffer actions are the same, or <tt>false</tt> otherwise
	 */
	 ...
	 */
	public int slotCount() { return slotActions.size(); }

	/** 
	 * Gets the slot action for a given index.
	 * @param i the slot action index
	 * @return the slot action at the given index
	 */
	public SlotAction getSlotAction (int i) { return slotActions.elementAt(i); }

	/** 
	 * Gets an iterator for all slot actions. 
	 * @return an iterator that iterates over all slot actions
	 */
	public Iterator<SlotAction> getSlotActions() { return slotActions.iterator(); }

	/** 
	 * Gets the slot action for a given slot.
	 * @param slot the slot
	 * @return the slot action for that slot, or <tt>null</tt> if the slot is not present 
	 */
	 ...
	 * @param slot the slot
	 * @returns <tt>true</tt> if the buffer action specifies the slot in any slot action, and <tt>false</tt> otherwise 
	 */
	public boolean hasSlotAction (Symbol slot) { return getSlotAction(slot)!=null; }

	/** 
	 * Checks whether the buffer action specifies a given slot value.
	 * @param value the slot value
	 * @returns <tt>true</tt> if the buffer action specifies the slot value in any slot action, and <tt>false</tt> otherwise 
	 */
	...
	/** 
	 * Checks whether the buffer action represents the direct setting of a buffer (as in <tt>=buffer> =chunk</tt>).
	 * @return <tt>true</tt> if the buffer action is a direct setting, or <tt>false</tt> otherwise
	 */
	public boolean isDirect () { return (directAction != null); }

	/** 
	 * Checks whether the buffer action is a special action denoted by the <tt>!</tt> prefix.
	 * @return <tt>true</tt> if the buffer action is special, or <tt>false</tt> otherwise
	 */
	...
	private void storeInMemory (Symbol buffer, Instantiation inst, boolean forceVisual)
	{
		...
		{
			if (model.verboseTrace) model.output ("declarative", "store chunk ["+bufferChunk.getName()+"] " + bufferChunk);
			Chunk newChunk = model.getDeclarative().add (bufferChunk);
			if (newChunk!=bufferChunk && model.verboseTrace) model.output ("declarative", "merged into ["+newChunk.getName()+"]");
			if (newChunk != bufferChunk) inst.replaceValue (bufferChunk.getName(), newChunk.getName());
		}
	}
	...
	void fire (Instantiation inst)
	{
		if (directAction != null)
		{
			...
			if (directSymbol == null) { model.outputWarning (directAction+" not a valid symbol"); return; }
			Chunk direct = model.getDeclarative().get (directSymbol);
			if (direct==null) direct = model.getBuffers().getBufferChunk(directSymbol);
			if (direct==null) { model.outputWarning (directAction+" -> "+directSymbol+" not a valid chunk"); return; }
			direct = direct.copy ();
		...
		{
			Chunk bufferChunk = model.getBuffers().get (buffer); // inst.get (Symbol.get("="+buffer));
			if (bufferChunk==null) { model.outputWarning (buffer+" empty, not referenced in LHS?"); return; }
			for (int i=0 ; i<slotActions.size() ; i++)
				slotActions.elementAt(i).fire (inst, bufferChunk);
		}
		else if (prefix == '+')
		{
			if (model.getDeclarative().addChunkOnNewRequest) storeInMemory (buffer, inst, false);
		...
		else if (prefix == '!')
		{
			...
			else
			{
				...
				} catch (Exception e) { model.getTask().eval (tokens.iterator()); }
			}
		}
	}
	...
	void expandDirectAction (Instantiation inst)
	{
		...
		if (chunk==null) chunk = model.getBuffers().getBufferChunk(name);
		// when used for production compilation, expansion doesn't always have the chunk in the buffer!
		// (particularly for the first production in compilation, which fired previously)
		...
		while (it.hasNext())
		{
			...
			slotActions.add (new SlotAction (model, slot,value));
		}
		directAction = null;
	}

	/** 
	 * Gets a string representation of the buffer action, with standard indentation. 
	 * @return the string
	 */
