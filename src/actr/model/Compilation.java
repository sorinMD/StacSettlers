 ...
 * This version represents the "simple" version that uses only a generic compilation
 * process for all buffers except the retrieval buffer, which still uses a special
 * compilation process.
 *  
...
class Compilation
{
	...
	boolean compileGoalStyle (Symbol buffer)
	{
		...
		else // if (p1.hasBufferAction('+',buffer) && !p2.hasBufferAction('+',buffer))
		{
	...
	boolean compileRetrievalStyle (Symbol buffer)
	{
		...
		while (it.hasNext())
		{
			...
			if (val != null) p1.specialize (var, val);
		}
		...
		while (it.hasNext())
		{
			...
			if (val != null) p2.specialize (var, val);
		}
		// NEW done
		...
		else
		{
			...
			{
				addCondition (p1.getBufferCondition(buffer));
				if (!newp.hasBufferCondition(buffer) // XXX bug? this fix added 5/20/11
				)
					addCondition (p1.getBufferCondition(stateBuffer));
			}
			else
		...
		while (itBC.hasNext())
		{
			...
			while (itSC.hasNext())
			{
				...
				if (sa1 != null)
				{
					...
					if (!value2.isVariable()) replaceValue (p1, inst1, value1, value2);
					else replaceValue (p2, inst2, value2, value1);
				}
			}
		}
	}
