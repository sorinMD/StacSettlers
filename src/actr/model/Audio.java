...
public class Audio extends Module
{
	...
	/**
	 * Adds an aural sound for access by the aural module. The identifier should be unique for different
	 * sounds; if the same identifier is used, the new sound will cancel the previously scheduled one.
	...
	public void addAural (final String id, String type, String content)
	{
		...
		if (auralloc != null)
		{
			...
			model.addEvent (new Event (model.getTime() + detectDelay,
					"audio", "audio-event ["+auralloc.getName()+"]") {
				public void action() {
					audicon.put (Symbol.get(id), ao);
					if (model.bufferStuffing && model.getBuffers().get(Symbol.aurloc)==null)
					{
						if (model.verboseTrace) model.output ("audio", "unrequested ["+auralloc.getName()+"]");
	...
	void update ()
	{
		...
		if (request!=null && request.isRequest())
		{
			...
			if (auralloc != null)
			{
				if (model.verboseTrace) model.output ("audio", "find-sound ["+auralloc.getName()+"]");
				model.getBuffers().set (Symbol.aurloc, auralloc);
				model.getBuffers().setSlot (Symbol.aurlocState, Symbol.state, Symbol.free);
		...
		if (request!=null && request.isRequest()) // && request.get(Symbol.isa)==Symbol.get("sound"))
		{
			...
			if (aurallocName == null) { model.outputWarning ("bad aural location"); return; }
			AuralObject ao = aurallocs.get (aurallocName);
			Chunk auralloc = ao.auralloc;
			if (auralloc == null) { model.outputWarning ("bad aural location"); return; }
			...
			model.addEvent (new Event (model.getTime() + recodeDelay, "audio", "audio-encoding-complete ["+aural.getName()+"]")
			{
				public void action() {
