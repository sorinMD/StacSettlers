...
class FindPanel extends JPanel
{
	...
	FindPanel (final Frame frame, final boolean forEditor)
	{
		...
		findTF.getDocument().addDocumentListener (new DocumentListener() {
			public void changedUpdate(DocumentEvent e) { find(true); update(); }
			public void insertUpdate(DocumentEvent e) { find(true); update(); }
			public void removeUpdate(DocumentEvent e) { find(true); update(); }
		});
		...
		left.add (findLabel);
		if (forEditor) left.add (replaceLabel);

		JPanel right = new JPanel();
		right.setLayout (new GridLayout(0,1));
		right.add (findTF);
		if (forEditor) right.add (replaceTF);
		...
		if (forEditor) buttons.add (replaceButton);
		if (forEditor) buttons.add (replaceAllButton);
		...
		addMouseListener (new MouseAdapter() {
			public void mouseEntered (MouseEvent e) { inPanel = true; repaint(); }
			public void mouseExited (MouseEvent e) { inPanel = false; repaint(); }
			public void mouseClicked(MouseEvent e)
			{
				Rectangle rect = new Rectangle (self.getWidth()-12, 0, 12, 12);
				if (rect.contains (e.getX(), e.getY())) frame.findHide();
			}
		});
		...
		setBorder (new AbstractBorder()
		{
			public void paintBorder (Component c, Graphics g, int x, int y, int width, int height)
			{
				...
				if (inPanel) g.drawString ("x", width-10, 10);
			}
			public Insets getBorderInsets (Component c) { return new Insets(6,18,6,12); }
		});
	...
	void find (boolean forward)
	{
		...
		if (forward)
		{
			...
			if (pos == -1) pos = text.indexOf (findText);
		}
		else
		{
			pos = text.lastIndexOf (findText, searchStart);
			if (pos == -1) pos = text.lastIndexOf (findText);
		}
	...
	void findNext ()
	{
		...
		if (searchStart >= textComponent.getText().length()) searchStart = 0;
		find (true);
	}
	...
	void findPrevious ()
	{
		JTextComponent textComponent = (forEditor) ? frame.getEditor() : frame.getOutputArea();
		searchStart = textComponent.getSelectionStart() - 1;
		if (searchStart < 0) searchStart = textComponent.getText().length() - 1;
		find (false);
	}
	...
	void replace ()
	{
		...
		if (lastPos==-1 || selectionLength==0) return;
	...
	void replaceAll ()
	{
		...
		if (pos >= 0)
		{
			...
			{
				try { frame.getDocument().replace (pos, findText.length(), replaceText, Document.styleNormal); }
				catch (Exception ex) { }
