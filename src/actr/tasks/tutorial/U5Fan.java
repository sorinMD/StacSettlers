...
public class U5Fan extends Task
{
	...
	String[][] tuples =
	{{"lawyer", "store", "t"}, {"captain", "cave", "t"}, {"hippie", "church", "t"}, 
	...
	public void start ()
	{
		if (lastRunUsedPerson) getModel().runCommand ("(spp retrieve-from-location :u 10)");
		else getModel().runCommand ("(spp retrieve-from-person :u 10)");
	...
	public void update (double time)
	{
		if (tupleIndex >= 0)
		{
			if (response == null) correct[tupleIndex] = false;
			else
			{
		...
		tupleIndex++;
		if (tupleIndex >= tuples.length) getModel().stop();
		else
		{
	...
	final double humanTimes[] =
	{1.11, 1.17, 1.22, 1.17, 1.20, 1.22, 1.15, 1.23, 1.36,
	...
	public Result analyze (Task[] tasks, boolean output)
	{
		...
		if (output)
		{
			...
			for (int i=0 ; i<tuples.length ; i+=3)
				getModel().output (String.format ("%.3f\t%.3f\t%.3f",
