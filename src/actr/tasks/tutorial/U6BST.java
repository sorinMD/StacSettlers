...
public class U6BST extends Task
{
	...
	{
		if (stimIndex >= 0) responses[stimIndex] = choice;
		stimIndex++;
	...
	double humanCounts[] = {20.0, 67.0, 20.0, 47.0, 87.0, 20.0, 80.0, 93.0,
			83.0, 13.0, 29.0, 27.0, 80.0, 73.0, 53.0};

	public int analysisIterations () { return 20; }
	...
	public Result analyze (Task[] tasks, boolean output)
	{
		...
		if (output)
		{
			...
			String s="decide-over"; getModel().output (String.format ("%-15s : %.3f", s, utilities[0]));
			s="decide-under"; getModel().output (String.format ("%-15s : %.3f", s, utilities[1]));
			s="force-over"; getModel().output (String.format ("%-15s : %.3f", s, utilities[2]));
			s="force-under"; getModel().output (String.format ("%-15s : %.3f", s, utilities[3]));
		}
