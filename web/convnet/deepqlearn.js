...
(function(global) {
  ...
  Brain.prototype = {
    ...
    policy: function(s) {
      ...
      var svol = new convnetjs.Vol(1, 1, this.net_inputs, 0.0);
      svol.w = s;
      ...
      //console.log("policy: indx_action_0="+indx_action_0 + " maxkey="+maxkey + " maxval="+maxval);
      for(var k=1;k<this.allowed_actions_array.length;k++) {
        ...
        //console.log("policy: indx_action_n="+indx_action_n + " key_action_n="+key_action_n + " val_action_n="+val_action_n);

        if(val_action_n>maxval) {
          maxkey = parseInt(indx_action_n); 
          maxval = val_action_n; 
        }
        //console.log("policy: key="+indx_action_n + " val="+val_action_n);
      }
    ...
    qvalues: function(input_array) {
      /*var net_input = this.getNetInput(input_array);
      var svol = new convnetjs.Vol(1, 1, this.net_inputs, 0.0);
      svol.w = net_input;
      var action_values = this.value_net.forward(svol);
      console.log("qvalues ... |W|="+action_values.length);
      for (var i=0; i<action_values.length; i++) {
        console.log("w["+i+"]="+action_values.w[i]);
      }
      return action_values.w;*/
    	
        var svol = new convnetjs.Vol(1, 1, this.net_inputs, 0.0);
    	var net_input = this.getNetInput(input_array);
        svol.w = net_input;
        var action_values = this.value_net.forward(svol);
        //console.log("qvalues: action_values="+action_values);
        var indx_action_0 = this.allowed_actions_array[0];
        var maxkey = parseInt(indx_action_0); 
        var maxval = action_values.w[maxkey];
        for(var k=1;k<this.allowed_actions_array.length;k++) {
          ...
          //console.log("key="+indx_action_n + " val="+val_action_n);
        }
        return action_values.w;
    },
    ...
    forward: function(input_array) {
      ...
      if(this.forward_passes > this.temporal_window) {
        ...
        if(rf < this.epsilon) {
          ...
          value = maxact.value;
          //console.log("forward: action="+maxact.action + " value="+maxact.value);
       }
      } else {
        ...
        action = this.random_action();
        value = -1;
        //console.log("forward: RANDOM action="+action);
      ...
      //return action;
      return {maxaction:action, maxvalue:value};
    },
    backward: function(reward) {
      ...
      if(this.experience.length > this.start_learn_threshold) {
        ...
        for(var k=0;k < this.tdtrainer.batch_size;k++) {
          ...
          var x = new convnetjs.Vol(1, 1, this.net_inputs, 0.0);
          x.w = e.state0;
